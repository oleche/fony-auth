          ...
          if ($verb == 'refresh'){
            if ($this->auth_handler->validateBasicToken($token)){
              if ($this->validate_fields($params, 'v1/authenticate/refresh', 'POST')){
                ...
                $this->response['refresh_token'] = $this->auth_handler->getApiToken()->columns['refresh_token'];
        				$this->response['expires'] = ((strtotime($this->auth_handler->getApiToken()->columns['updated_at'])*1000)+$this->auth_handler->getApiToken()->columns['expires']) - (time()*1000);
        				return true;
              }else{
                return false;
              }
