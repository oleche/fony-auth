...
use Geekcow\FonyAuth\Model\ApiAssetType;
...
class UserUpload
{
    ...
    {
        if (!AllowCore::isAllowed($scope, $this->validScope)) {
    ...
    private function prepareAsset($mime, $id, &$filepath, &$filename, &$tip, $oldfile)
    {
        $validation = false;
